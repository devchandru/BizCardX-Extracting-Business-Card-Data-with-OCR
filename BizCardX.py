...
# image to text converting format
...
def extracted_text(texts):
  ...
  for i in range(2,len(texts)):
    ...
    if texts[i].startswith("+") or (texts[i].replace("-", "").isdigit() and  '-' in texts[i]):
      ...
      extrd_dict["ADDRESS"].append(remove_colone)
...
with st.sidebar:
    select = option_menu("Main Menu", ["Home", "Upload & Modifying", "Delete", "About"])

if select == "Home":

  image1= Image.open("/content/download.jpg")
  ...
  if img is not None:
    ...
    image_date = Image_bytes.getvalue()


    # creating dictionary

    data = {"IMAGE":[image_date]}
    ...
    if button_1:
        ...
        create_table_queryt = '''CREATE TABLE IF NOT EXISTS bizcard_details(name varchar(250),
        ...
        cursor.execute(create_table_queryt)
        mydb.commit()


        # insert query

        insert_quary = '''INSERT INTO bizcard_details(name, designation, company_name, contact,
        ...
        cursor.execute(insert_quary, datas)
        mydb.commit()
  ...
  with col1:
    ...
    name_select = st.selectbox("Selectt the name", names)
  ...
  with col2:
    ...
    designation_select = st.selectbox("Selectt the designations", designations)
